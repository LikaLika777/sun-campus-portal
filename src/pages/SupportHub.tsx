...
const tiles = [
  { icon: BookOpen, title: "Academic Support", description: "Tutoring, study skills, special consideration and course advice.", href: "/support", color: "text-accent" },
  { icon: DollarSign, title: "Fees & Payments", description: "Payment plans, refunds, and fee-related questions.", href: "/support", color: "text-accent" },
  ...
  { icon: AlertTriangle, title: "Complaints & Appeals", description: "Formal complaints, academic appeals, and grievance processes.", href: "/support/complaint", color: "text-primary" },
  ...
  { icon: HelpCircle, title: "General Enquiry", description: "Any other question — we'll make sure it reaches the right team.", href: "/enquire", color: "text-accent" },
];
    ...
    <div>
      <section className="bg-accent text-accent-foreground py-12">
        <div className="container">
          <h1 className="font-heading text-3xl md:text-4xl font-extrabold mb-2 text-accent-foreground">How can we help?</h1>
        ...
        <motion.div variants={container} initial="hidden" animate="visible" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {tiles.map((t) => (
            <motion.div key={t.title} variants={item}>
              <Link to={t.href}>
                <Card className="h-full hover:shadow-md transition-all hover:-translate-y-0.5 cursor-pointer group border">
